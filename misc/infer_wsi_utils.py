...
def generate_patch_list(ano, patch_size, stride):
    max_height, max_width = ano.shape
    h_list = np.arange(0, max_height - patch_size, stride)
    w_list = np.arange(0, max_width - patch_size, stride)
    out = [[[h_list[h], w_list[w]] for w in range(len(w_list))] for h in range(len(h_list))]
    return list(chain(*out))


def generate_patch_list_(ano, roi, patch_size, stride):
